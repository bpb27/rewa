...
export const getByJob = (
  ...
  const matchesJob = {
    ...
    writer: (crew: GetByJobCrew) => crew.job === "Screenplay" || crew.job === "Writer",
...
export type HashUpsertContainer<TEntry extends { id: number }, TMovie extends { id: number }> = {
  [id: number]: {
    total: number;
    movies: TMovie[];
  } & TEntry;
};
export const hashUpsert = <TEntry extends { id: number }, TMovie extends { id: number }>(
